...
export async function PUT({ request, params, locals }) {
    ...
    try {
        ...
        const [result] = await pool.query(
            'UPDATE bills_reminder SET bill_title = ?, bill_amount = ?, due_date = ?, recurring_bill = ?, isPaid = ? WHERE id = ? AND user_id = ?',
            [bill_title, bill_amount, due_date, recurring_bill, isPaid, params.id, locals.user.id]
        );
